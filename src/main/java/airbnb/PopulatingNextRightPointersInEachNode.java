 ...
 *
 * 第 NN 层节点之间建立 \text{next}next 指针后，再建立第 N+1N+1 层节点的 \text{next}next 指针。可以通过 \text{next}next 指针访问同一层的所有节点，因此可以使用第 NN 层的 \text{next}next 指针，为第 N+1N+1 层节点建立 \text{next}next 指针。
 *
 * 算法
 *
 * 从根节点开始，由于第 00 层只有一个节点，所以不需要连接，直接为第 11 层节点建立 \text{next}next 指针即可。该算法中需要注意的一点是，当我们为第 NN 层节点建立 \text{next}next 指针时，处于第 N-1N−1 层。当第 NN 层节点的 \text{next}next 指针全部建立完成后，移至第 NN 层，建立第 N+1N+1 层节点的 \text{next}next 指针。
 *
 * 遍历某一层的节点时，这层节点的 \text{next}next 指针已经建立。因此我们只需要知道这一层的最左节点，就可以按照链表方式遍历，不需要使用队列。
 *
 * 上面思路的伪代码如下：
 *
 *
 ...
 *
 *
 * 两种类型的 \text{next}next 指针。
 *
 * 第一种情况两个子节点属于同一个父节点，因此直接通过父节点建立两个子节点的 \text{next}next 指针即可。
 *
 *
 * node.left.next = node.right
 *
